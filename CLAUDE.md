...
**Status: every entry helper in the public surface is rewritten end-to-end.** Bare `q.Try` / `q.NotNil` plus all five `q.TryE(...).Method(...)` and all five `q.NotNilE(...).Method(...)` chain methods. Pending — each unsupported shape emits a diagnostic so half-rewritten builds never happen silently:

- Discard form: `q.Try(call())` (no LHS) — pending (task #13).
- Plain assignment: `v = q.Try(call())` (`=`, not `:=`) — pending (task #13).
