...
- `internal/preprocessor/scanner.go` — recognises `q.*` call expressions on the AST. **Matches all four entries × four statement forms:** define (`v := …`), assign (`v = …`, including non-ident LHS like `obj.field`), discard (no-LHS expression statement), return (`return …, q.Try(…), …` — and q.* can be nested anywhere in a result expression; the scanner walks each result's subtree with `ast.Inspect` to locate every q.* call). Each entry has a bare and chain (`.Err`/`.ErrF`/`.Catch`/`.Wrap`/`.Wrapf`) variant. Resolves the local import alias of `pkg/q` per file. Any unmatched `q.*` reference in a user file becomes a diagnostic. **Per-statement** fields live on `callShape` (Stmt, Form, LHSExpr, EnclosingFuncType); **per-call** fields live on `qSubCall` (Family, Method, MethodArgs, InnerExpr, OuterCall). A callShape owns `Calls []qSubCall` — length 1 for non-return forms, 1..N for return (a single `return q.Try(a()) + q.Try(b()), nil` is one shape with two sub-calls). `matchStatement` dispatches on the statement kind, `classifyQCall` returns a qSubCall, and the return-case scanner collects multiple sub-calls into one callShape. **Closure support:** `walkFuncLits` descends into every `*ast.FuncLit` reachable without crossing a nested `*ast.BlockStmt`, and recursive `walkBlock` calls track the nearest-enclosing function signature via `EnclosingFuncType *ast.FuncType` — so q.* inside a closure bubbles according to the closure's own result list, not the outer FuncDecl's.
- `internal/preprocessor/rewriter.go` — emits replacement source for one matched statement. `renderShape` iterates `sh.Calls`, rendering each sub-call via `renderSubCall` as a bind + bubble block, and joining them with a newline + indent. For formReturn, it then appends `finalReturnSuffix` which rebuilds the original return with every q.* span substituted by its own `_qTmp<N>` (multiple substitutions applied bottom-up so offsets stay valid). Per-family bind line varies by form (`tryBindLine` for the Try family, `nilBindLineAndCheck` for the NotNil family); the return form binds to `_qTmp<N>` regardless of family. Each sub-call gets its own counter value so `_qErrN` / `_qTmpN` / `_qValN` / `_qRetN` are globally unique within a file. Try family bubbles on `err != nil`; NotNil bubbles on `<checkVar> == nil`. The discard form binds to a fresh `_qVal<N>` for NotNil (no LHS to reuse) and `_, _qErr<N>` for Try. Bare and most chain methods share family-specific skeletons (`assembleErrBlock` / `assembleNilBlock`); `Catch` has its own assembler in each family because the err / nil branch may either recover (rebind the LHS via fn) or transform (bubble a new error). For discard form, Catch's recovery position becomes `_` since there's nowhere to put a recovered value. NotNil's `Wrap`/`Wrapf` differ from TryE's: there's no source error to %w-wrap, so they emit `errors.New(msg)` / `fmt.Errorf(format, args...)` respectively. Uses the universal `*new(T)` zero-value form. When a render needs `fmt` or `errors`, the rewriter calls `ensureImport` to inject the package into the file's import block (parenthesised form preferred; single-line and zero-import cases handled separately). Always appends a `var _ = <alias>.ErrNil` sentinel so the q import stays alive after rewrites erase the only callers.
...
**Status: the full public surface is rewritten end-to-end across four statement forms.** Define (`v := ...`), assign (`v = ...`), discard (`q.Try(...)` as expression statement), and return (`return q.Try(...), nil`, including q.* nested inside a return-result expression like `return q.Try(call()) * 2, nil`) all work for bare `q.Try` / `q.NotNil` and for every `q.TryE` / `q.NotNilE` chain method. Pending — each unsupported shape still emits a diagnostic so half-rewritten builds never happen silently:

- Nested-in-call outside return-position (`f(q.Try(call()))`) — still pending. The return-position path handles q.* nested inside return-result expressions; this more general case (hoisting into a preceding statement) is tracked in `docs/planning/TODO.md` #16.
- Multi-LHS — out of scope until q.Try3 / multi-T runtime helpers exist.
