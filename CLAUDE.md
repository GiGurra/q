...
**Status: the full public surface is rewritten end-to-end across all three statement forms.** Define (`v := ...`), assign (`v = ...`), and discard (`q.Try(...)` as expression statement) all work for bare `q.Try` / `q.NotNil` and for every `q.TryE` / `q.NotNilE` chain method. Pending — each unsupported shape still emits a diagnostic so half-rewritten builds never happen silently:

- Return-position (`return q.Try(call())`) — out of scope per `docs/design.md` §4.4.
- Nested-in-call (`f(q.Try(call()))`) — out of scope.
- Multi-LHS — out of scope.
